...
[HubSpot](https://www.hubspot.com/) is a cloud-based customer relationship management (CRM) platform that helps businesses grow by providing tools for marketing, sales, customer service, and content management, all powered by AI to drive customer engagement and revenue.

The `ballerinax/hubspot.crm.engagements.email` package offers APIs to connect and interact with [HubSpot CRM Engagements Email API](https://developers.hubspot.com/docs/reference/api/crm/engagements/email) endpoints, specifically based on [HubSpot API v3](https://developers.hubspot.com/docs/reference/api).
## Setup guide
...
2. If you intend to use private apps for API access, note that this feature is available across all HubSpot plans including the free tier. However, certain API endpoints and scopes may require a Professional or Enterprise plan subscription depending on the specific CRM features you need to access.
...
6. Navigate to the Scopes tab and select the required scopes for email engagements (such as `crm.objects.contacts.read`, `crm.objects.contacts.write`, and `sales-email-read`).
