...
[HubSpot](https://www.hubspot.com/) is a cloud-based customer relationship management (CRM) platform that helps businesses grow by connecting marketing, sales, and customer service teams with the tools they need to attract, engage, and delight customers.
...
To use the HubSpot CRM Engagements Email connector, you must have access to the HubSpot API through a [HubSpot developer account](https://developers.hubspot.com/) and obtain an API access token. If you do not have a HubSpot account, you can sign up for one [here](https://www.hubspot.com/products/get-started).
...
2. Note that while HubSpot offers free tools, certain API features and higher rate limits require a Professional or Enterprise plan subscription. Review your plan's API access capabilities to ensure compatibility with your integration needs.

### Step 2: Generate an API Access Token

1. Log in to your HubSpot account.

2. Click the settings icon (gear icon) in the main navigation bar.
...
6. Navigate to the Scopes tab and select the required scopes for CRM engagements and email access (such as `crm.objects.contacts.read`, `crm.objects.contacts.write`, and `sales-email-read`).

7. Click Create app in the top right corner, then review and confirm by clicking Continue creating.
