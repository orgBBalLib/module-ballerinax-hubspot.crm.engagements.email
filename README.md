...
The `ballerinax/hubspot.crm.engagements.email` package offers APIs to connect and interact with [HubSpot CRM Engagements Email API](https://developers.hubspot.com/docs/reference/api/crm/engagements/email) endpoints, specifically based on [HubSpot CRM API v3](https://developers.hubspot.com/docs/reference/api).
## Setup guide
...
6. Navigate to the Scopes tab and select the required scopes for CRM engagements. For email engagements, enable scopes such as `crm.objects.contacts.read`, `crm.objects.contacts.write`, and `sales-email-read`.

7. Click Create app in the top right corner, then review the details and click Continue creating.

8. Once created, your access token will be displayed. Click Show token to reveal and copy it.

> **Tip:** You must copy and store this key somewhere safe. It won't be visible again without regenerating for security reasons.
## Quickstart
